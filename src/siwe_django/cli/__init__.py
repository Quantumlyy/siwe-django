...
``settings.py`` and the root ``urls.py`` via libcst, optionally drops a
starter Django template, and runs ``manage.py migrate`` so adopters can sign
in with Ethereum within a minute of ``pip install siwe-django[cli]``.
