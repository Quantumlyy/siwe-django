...
directly and posts to the standard `siwe-django` endpoints.

This is the no-JS-toolchain counterpart to `examples/showcase/` and the
output the `siwe-django scaffold-templates` wizard step would produce
when run against an empty project.
...
- `templates_demo/urls.py` mounts the SIWE endpoints under `/auth/siwe/`
  and renders the bundled `siwe_django/siwe_login.html` at `/`.
